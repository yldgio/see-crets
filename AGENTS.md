...
Key paths: src/vault/ (backends), src/tools/ (LLM-callable), src/lifecycle.ts (human-only: delete/purge/rotate), src/utils/ (git root detection), src/runtimes/ (per-runtime plugins), hooks/ (Tier 3 enforcement)
