...
import { LinuxVaultBackend } from "./linux.ts";
...
// ---------------------------------------------------------------------------
// LinuxVaultBackend unit tests (key validation — no OS calls for these)
// ---------------------------------------------------------------------------

describe("LinuxVaultBackend key validation", () => {
...
// ---------------------------------------------------------------------------
// LinuxVaultBackend fallback chain tests
// These run on any OS by injecting a mock probe function.
// ---------------------------------------------------------------------------

describe("LinuxVaultBackend fallback chain", () => {
  it("picks secret-tool when both tools are available", async () => {
    const backend = new LinuxVaultBackend();
    // Both available — secret-tool is first in chain
    const tool = await backend.resolveBackend((_cmd) => true);
    expect(tool).toBe("libsecret");
  });

  it("falls back to pass when secret-tool is unavailable", async () => {
    const backend = new LinuxVaultBackend();
    const tool = await backend.resolveBackend((cmd) => cmd === "pass");
    expect(tool).toBe("pass");
  });

  it("throws when neither tool is available", async () => {
    const backend = new LinuxVaultBackend();
    await expect(
      backend.resolveBackend((_cmd) => false)
    ).rejects.toThrow("No vault backend available");
  });

  it("isAvailable() returns true when secret-tool is available", async () => {
    // isAvailable() calls resolveBackend() with the real probe.
    // We test the method by monkey-patching resolveBackend on a fresh instance.
    const backend = new LinuxVaultBackend();
    backend.resolveBackend = async () => "libsecret";
    expect(await backend.isAvailable()).toBe(true);
  });

  it("isAvailable() returns false when neither tool is available", async () => {
    const backend = new LinuxVaultBackend();
    backend.resolveBackend = async () => {
      throw new Error("No vault backend available");
    };
